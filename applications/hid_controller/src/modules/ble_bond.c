...
LOG_MODULE_REGISTER(MODULE, CONFIG_CONTROLLER_BLE_BOND_LOG_LEVEL);
...
#if CONFIG_CONTROLLER_BLE_PEER_SELECT
...
#if CONFIG_CONTROLLER_BLE_NEW_PEER_SCAN_REQUEST
	{STATE_IDLE,        CLICK_SHORT,  STATE_IDLE,        scan_request},
#endif

#if CONFIG_CONTROLLER_BLE_PEER_ERASE
...
#endif /* CONFIG_CONTROLLER_BLE_PEER_ERASE */

#if CONFIG_CONTROLLER_BLE_PEER_ERASE_ON_START
...
#if CONFIG_CONTROLLER_BLE_DONGLE_PEER_ENABLE
...
static void swap_bt_stack_peer_id(void)
{
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_BLE_USE_DEFAULT_ID)) {
...
static void scan_request(void)
{
	if (!IS_ENABLED(CONFIG_CONTROLLER_BLE_NEW_PEER_SCAN_REQUEST)) {
...
static void silence_unused(void)
{
	...
	if (!IS_ENABLED(CONFIG_CONTROLLER_BLE_PEER_ERASE)) {
...
static int init(void)
{
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_BLE_PEER_CONTROL)) {
...
{
	if (event->key_id != CONFIG_CONTROLLER_BLE_PEER_CONTROL_BUTTON) {
		return false;
	}
...
{
	if (event->selector_id != CONFIG_CONTROLLER_BLE_DONGLE_PEER_SELECTOR_ID) {
		return;
	}

	if (event->position == CONFIG_CONTROLLER_BLE_DONGLE_PEER_SELECTOR_POS) {
		switch (state) {
		case STATE_DISABLED:
...
static bool event_handler(const struct event_header *eh)
{
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_BLE_DONGLE_PEER_ENABLE) &&

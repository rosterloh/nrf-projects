...
LOG_MODULE_REGISTER(MODULE, CONFIG_CONTROLLER_HID_STATE_LOG_LEVEL);
...
#define SUBSCRIBER_COUNT (IS_ENABLED(CONFIG_CONTROLLER_HIDS_ENABLE) + \
			  IS_ENABLED(CONFIG_CONTROLLER_USB_ENABLE))

#define INPUT_REPORT_DATA_COUNT (IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_MOUSE_SUPPORT) +		\
				 IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_KEYBOARD_SUPPORT) +	\
				 IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_SYSTEM_CTRL_SUPPORT) +	\
				 IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_CONSUMER_CTRL_SUPPORT))

#define INPUT_REPORT_STATE_COUNT (IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_MOUSE_SUPPORT) +		\
				  IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_KEYBOARD_SUPPORT) +	\
				  IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_SYSTEM_CTRL_SUPPORT) +	\
				  IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_CONSUMER_CTRL_SUPPORT) +	\
				  IS_ENABLED(CONFIG_CONTROLLER_HID_BOOT_INTERFACE_MOUSE) +		\
				  IS_ENABLED(CONFIG_CONTROLLER_HID_BOOT_INTERFACE_KEYBOARD))
...
#define AXIS_COUNT (IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_MOUSE_SUPPORT) * MOUSE_REPORT_AXIS_COUNT)
...
{
	return (eventq->len >= CONFIG_CONTROLLER_HID_EVENT_QUEUE_SIZE);
}
...
static void eventq_cleanup(struct eventq *eventq, u32_t timestamp)
{
	...
	SYS_SLIST_FOR_EACH_NODE(&eventq->root, first_valid) {
		...
		if (diff < CONFIG_CONTROLLER_HID_REPORT_EXPIRATION) {
			break;
		}
	}
...
{
	__ASSERT_NO_MSG((IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_KEYBOARD_SUPPORT) &&
	...
	if (!IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_KEYBOARD_SUPPORT)) {
		/* Not supported. */
...
static void send_report_mouse(u8_t report_id, struct report_data *rd)
{
	...
	if (!IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_MOUSE_SUPPORT)) {
		/* Not supported. */
		__ASSERT_NO_MSG(false);
		return;
	}
...
static void send_report_boot_mouse(u8_t report_id, struct report_data *rd)
{
	...
	if (!IS_ENABLED(CONFIG_CONTROLLER_HID_BOOT_INTERFACE_MOUSE)) {
		/* Not supported. */
		__ASSERT_NO_MSG(false);
		return;
	}
			...
			SYS_SLIST_FOR_EACH_NODE(&rd->eventq.root, i) {
				...
				u32_t timestamp =
					CONTAINER_OF(i,
						     struct item_event,
						     node)->timestamp +
					CONFIG_CONTROLLER_HID_REPORT_EXPIRATION;
...
static void init(void)
{
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_SYSTEM_CTRL_SUPPORT)) {
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_CONSUMER_CTRL_SUPPORT)) {
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_HID_BOOT_INTERFACE_KEYBOARD)) {
...
{
	if (!IS_ENABLED(CONFIG_CONTROLLER_MOTION_NONE) &&
	    is_motion_event(eh)) {
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_WHEEL_ENABLE) &&
	    is_wheel_event(eh)) {
		return handle_wheel_event(cast_wheel_event(eh));
	}

	if (!IS_ENABLED(CONFIG_CONTROLLER_BUTTONS_NONE) &&
	    is_button_event(eh)) {
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_USB_ENABLE) &&

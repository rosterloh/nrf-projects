...
static bool event_handler(const struct event_header *eh)
{
...
#if CONFIG_SHELL_BACKEND_SERIAL
                        shell_prompt_change(shell_backend_serial_get_ptr(),
					    "nrf_controller>");
#else
	...
	if (is_wake_up_event(eh)) {
		if (!enabled) {
#if CONFIG_SHELL_BACKEND_SERIAL
                        shell_start(shell_backend_serial_get_ptr());
#else
	...
	if (is_power_down_event(eh)) {
		if (enabled) {
#if CONFIG_SHELL_BACKEND_SERIAL
                        shell_stop(shell_backend_serial_get_ptr());
#else

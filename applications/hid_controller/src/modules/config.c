...
LOG_MODULE_REGISTER(MODULE, CONFIG_CONTROLLER_CONFIG_LOG_LEVEL);

#define KEY_LEN 11

#define THREAD_STACK_SIZE	CONFIG_CONTROLLER_SETTINGS_LOAD_THREAD_STACK_SIZE
...
static bool config_id_is_supported(u8_t event_id)
{
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_MOTION_SENSOR_ENABLE)) {
		if (MOD_FIELD_GET(event_id) == SETUP_MODULE_SENSOR) {
			return true;
		}
	}

	if (IS_ENABLED(CONFIG_CONTROLLER_BLE_QOS_ENABLE)) {
...
static void init_settings(void)
{
	...
	if (err) {
		...
		module_set_state(MODULE_STATE_ERROR);
	} else if (IS_ENABLED(CONFIG_CONTROLLER_SETTINGS_LOAD_BY_THREAD_ENABLE)) {
...
static bool module_event_handler(const struct module_state_event *event)
{
	...
	const void * const req_modules[] = {
		...
		MODULE_ID(bas),
#endif
#if CONFIG_CONTROLLER_BLE_ADVERTISING_ENABLE
		MODULE_ID(ble_adv),
#endif
#if CONFIG_CONTROLLER_MOTION_SENSOR_ENABLE
		MODULE_ID(motion),
#endif
#if CONFIG_CONTROLLER_BLE_SCANNING_ENABLE
		MODULE_ID(ble_scan),
#endif
	};

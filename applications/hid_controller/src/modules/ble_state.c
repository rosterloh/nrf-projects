...
LOG_MODULE_REGISTER(MODULE, CONFIG_CONTROLLER_BLE_STATE_LOG_LEVEL);
...
{
	__ASSERT_NO_MSG(IS_ENABLED(CONFIG_CONTROLLER_BLE_ENABLE_PASSKEY));
...
static void set_tx_power(struct bt_conn *conn)
{
	...
	if (err) {
		...
		cp->tx_power_level = CONFIG_CONTROLLER_BLE_TX_PWR;
...
static void disconnected(struct bt_conn *conn, u8_t reason)
{
	...
	if (IS_ENABLED(CONFIG_CONTROLLER_BLE_ENABLE_PASSKEY)) {
...
#if CONFIG_CONTROLLER_BLE_ENABLE_PASSKEY
EVENT_SUBSCRIBE(MODULE, passkey_input_event);
#endif /* CONFIG_CONTROLLER_BLE_ENABLE_PASSKEY */

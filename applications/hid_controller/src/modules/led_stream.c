...
LOG_MODULE_REGISTER(MODULE, CONFIG_CONTROLLER_LED_STREAM_LOG_LEVEL);
...
#define STEPS_QUEUE_ARRAY_SIZE (CONFIG_CONTROLLER_LED_STREAM_QUEUE_SIZE + 1)
...
static struct led leds[CONFIG_CONTROLLER_LED_COUNT];
...
{
	static const size_t min_len = CONFIG_CONTROLLER_LED_COLOR_COUNT * sizeof(led->steps_queue[led->rx_idx].color.c[0])

...
LOG_MODULE_REGISTER(MODULE, CONTROLLER_CONTROLLER_WATCHDOG_LOG_LEVEL);

#define WDT_FEED_WORKER_DELAY_MS ((CONFIG_CONTROLLER_WATCHDOG_TIMEOUT)/3)
...
static int watchdog_timeout_install(struct wdt_data_storage *data)
{
	static const struct wdt_timeout_cfg wdt_settings = {
			.window = {
				.min = 0,
				.max = CONFIG_CONTROLLER_WATCHDOG_TIMEOUT
			},
			.callback = NULL,
	...
	LOG_INF("Watchdog timeout installed. Timeout: %d", CONFIG_CONTROLLER_WATCHDOG_TIMEOUT);
	return 0;
}

...
LOG_MODULE_REGISTER(MODULE, CONFIG_CONTROLLER_BLE_QOS_LOG_LEVEL);

#define INVALID_BLACKLIST 0xFFFF

#if CONFIG_CONTROLLER_BLE_QOS_STATS_PRINTOUT_ENABLE
...
#define THREAD_STACK_SIZE		     \
	(CONFIG_CONTROLLER_BLE_QOS_STACK_SIZE + \
	 CONFIG_CONTROLLER_BLE_QOS_STATS_PRINT_STACK_SIZE)
#else
#define THREAD_STACK_SIZE CONFIG_CONTROLLER_BLE_QOS_STACK_SIZE
#endif
...
static void ble_chn_stats_print(bool update_channel_map)
{
	...
	if (!IS_ENABLED(CONFIG_CONTROLLER_BLE_QOS_STATS_PRINTOUT_ENABLE)) {
		return;
	}
...
{
	if (IS_ENABLED(CONFIG_CONTROLLER_BLE_QOS_STATS_PRINTOUT_ENABLE) &&
	    IS_ENABLED(CONFIG_CONTROLLER_HID_REPORT_MOUSE_SUPPORT)) {
...
static void ble_qos_thread_fn(void)
{
	while (true) {
		...
		k_sleep(CONFIG_CONTROLLER_BLE_QOS_INTERVAL);

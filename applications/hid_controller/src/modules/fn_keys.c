...
LOG_MODULE_REGISTER(MODULE, CONFIG_CONTROLLER_FN_KEYS_LOG_LEVEL);
...
static u16_t fn_key_pressed[CONFIG_CONTROLLER_FN_KEYS_MAX_ACTIVE];
...
#ifdef CONFIG_CONTROLLER_STORE_FN_LOCK
SETTINGS_STATIC_HANDLER_DEFINE(fn_keys, MODULE_NAME, NULL, settings_set, NULL,
			       NULL);

#endif /* CONFIG_CONTROLLER_STORE_FN_LOCK */
...
{
	if (IS_ENABLED(CONFIG_CONTROLLER_STORE_FN_LOCK)) {
...
{
	if (unlikely(event->key_id == CONFIG_CONTROLLER_FN_KEYS_SWITCH)) {
		fn_switch_active = event->pressed;
		return true;
	}

	if (unlikely(event->key_id == CONFIG_CONTROLLER_FN_KEYS_LOCK)) {
		if (event->pressed) {

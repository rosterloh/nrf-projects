...
LOG_MODULE_REGISTER(MODULE, CONFIG_CONTROLLER_WHEEL_LOG_LEVEL);


#define SENSOR_IDLE_TIMEOUT K_SECONDS(CONFIG_CONTROLLER_WHEEL_SENSOR_IDLE_TIMEOUT)
...
static void data_ready_handler(struct device *dev, struct sensor_trigger *trig)
{
	...
	if (!IS_ENABLED(CONFIG_CONTROLLER_WHEEL_INVERT_AXIS)) {
		wheel *= -1;
	}

	BUILD_ASSERT_MSG(CONFIG_CONTROLLER_WHEEL_SENSOR_VALUE_DIVIDER > 0,
			 "Divider must be non-negative");
	if (CONFIG_CONTROLLER_WHEEL_SENSOR_VALUE_DIVIDER > 1) {
		wheel /= CONFIG_CONTROLLER_WHEEL_SENSOR_VALUE_DIVIDER;
	}

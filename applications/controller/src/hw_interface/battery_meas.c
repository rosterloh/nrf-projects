...
LOG_MODULE_REGISTER(MODULE, CONFIG_CONTROLLER_BATTERY_MEAS_LOG_LEVEL);
 ...
 */
#define BATTERY_WORK_INTERVAL	(CONFIG_CONTROLLER_BATTERY_MEAS_POLL_INTERVAL_MS / 2)

#if IS_ENABLED(CONFIG_CONTROLLER_BATTERY_MEAS_HAS_VOLTAGE_DIVIDER)
#define BATTERY_VOLTAGE(sample)	(sample * BATTERY_MEAS_VOLTAGE_GAIN	\
		* ADC_REF_INTERNAL_MV					\
		* (CONFIG_CONTROLLER_BATTERY_MEAS_VOLTAGE_DIVIDER_UPPER	\
		+ CONFIG_CONTROLLER_BATTERY_MEAS_VOLTAGE_DIVIDER_LOWER)	\
		/ CONFIG_CONTROLLER_BATTERY_MEAS_VOLTAGE_DIVIDER_LOWER / ADC_MAX)
#else
#define BATTERY_VOLTAGE(sample) (sample * BATTERY_MEAS_VOLTAGE_GAIN	\
...
{
	if (IS_ENABLED(CONFIG_CONTROLLER_BATTERY_MEAS_HAS_ENABLE_PIN)) {
		int err = gpio_pin_write(gpio_dev,
					 CONFIG_CONTROLLER_BATTERY_MEAS_ENABLE_PIN,
					 1);
		if (err) {
...
static void battery_lvl_process(void)
{
	...
	if (voltage > CONFIG_CONTROLLER_BATTERY_MEAS_MAX_LEVEL) {
		level = 100;
	} else if (voltage < CONFIG_CONTROLLER_BATTERY_MEAS_MIN_LEVEL) {
		level = 0;
		LOG_WRN("Low battery");
	} else {
		/* Using lookup table to convert voltage[mV] to SoC[%] */
		size_t lut_id = (voltage - CONFIG_CONTROLLER_BATTERY_MEAS_MIN_LEVEL
				 + (CONFIG_CONTROLLER_VOLTAGE_TO_SOC_DELTA >> 1))
				 / CONFIG_CONTROLLER_VOLTAGE_TO_SOC_DELTA;

...
LOG_MODULE_REGISTER(MODULE, CONFIG_NW_SNS_BUTTONS_LOG_LEVEL);

#define SCAN_INTERVAL CONFIG_NW_SNS_BUTTONS_SCAN_INTERVAL
...
static int get_buttons(u32_t *mask)
{
	for (size_t i = 0; i < ARRAY_SIZE(button_pins); i++) {
		...
		val = gpio_pin_get_raw(gpio_devs[button_pins[i].port],
				       button_pins[i].pin);

		if (IS_ENABLED(CONFIG_NW_SNS_BUTTONS_POLARITY_INVERSED)) {
			val = !val;
		}
...
static int callback_ctrl(bool enable)
{
	...
	for (size_t i = 0; (i < ARRAY_SIZE(button_pins)) && !err; i++) {
		if (enable) {
			err = gpio_enable_callback(gpio_devs[button_pins[i].port],
						   button_pins[i].pin);
		} else {
			err = gpio_disable_callback(gpio_devs[button_pins[i].port],
						    button_pins[i].pin);
		}
	}
	if (!enable) {
...
static void scan_fn(struct k_work *work)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(button_pins); i++) {
		bool is_pressed = raw_state & BIT(i);
		bool was_pressed = settled_state & BIT(i);

		if ((is_pressed != was_pressed) &&
		    (evt_limit < CONFIG_NW_SNS_BUTTONS_EVENT_LIMIT)) {
			...
			WRITE_BIT(settled_state, i, is_pressed);
		}
	}
			       ...
			       u32_t pins)
{
	...
	for (u32_t pin = 0; (pins != 0) && !err; pins >>= 1, pin++) {
		if ((pins & 1) != 0) {
			err = gpio_disable_callback(gpio_dev, pin);
		}
	}
...
static void button_pressed_fn(struct k_work *work)
{
	...
	case STATE_ACTIVE:
		...
		k_delayed_work_submit(&buttons_scan,
				      CONFIG_NW_SNS_BUTTONS_DEBOUNCE_INTERVAL);
		break;
...
static void init_fn(void)
{
	...
	int flags = (IS_ENABLED(CONFIG_NW_SNS_BUTTONS_POLARITY_INVERSED) ?
		    (GPIO_PULL_UP | GPIO_INT_LOW_0) :
		    (GPIO_PULL_DOWN | GPIO_INT_HIGH_1));
	flags |= (GPIO_INPUT | GPIO_INT_ENABLE | GPIO_INT_EDGE);

	int err = 0;

	for (size_t i = 0; (i < ARRAY_SIZE(button_pins)) && !err; i++) {
		__ASSERT_NO_MSG(button_pins[i].port < ARRAY_SIZE(port_map));
		__ASSERT_NO_MSG(port_map[button_pins[i].port] != NULL);

		err = gpio_pin_configure(gpio_devs[button_pins[i].port],
				         button_pins[i].pin, flags);
	}
	...
	for (size_t i = 0; i < ARRAY_SIZE(button_pins); i++) {
		...
		err = gpio_disable_callback(gpio_devs[button_pins[i].port],
					    button_pins[i].pin);
		if (err) {
			LOG_ERR("Cannot configure buttons");
			goto error;
		}

		pin_mask[button_pins[i].port] |= BIT(button_pins[i].pin);
	}
	...
	for (size_t i = 0; i < ARRAY_SIZE(port_map); i++) {
		...
		gpio_init_callback(&gpio_cb[i], button_pressed_isr,
				   pin_mask[i]);

...
LOG_MODULE_REGISTER(MODULE, CONFIG_NW_SNS_SETTINGS_LOADER_LOG_LEVEL);

#define THREAD_STACK_SIZE	CONFIG_NW_SNS_SETTINGS_LOADER_THREAD_STACK_SIZE
...
{
	if (IS_ENABLED(CONFIG_NW_SNS_SETTINGS_LOADER_USE_THREAD)) {
...
static bool module_event_handler(const struct module_state_event *event)
{
	...
	const void * const req_modules[] = {
		MODULE_ID(main),
#if CONFIG_NW_SNS_ENVIRO_ENABLE
		MODULE_ID(enviro),
#endif
	};
	...
	for (size_t i = 0; i < ARRAY_SIZE(req_modules); i++) {
		if (check_state(event, req_modules[i], MODULE_STATE_READY)) {
			...
			/* Catch double initialisation of any module */

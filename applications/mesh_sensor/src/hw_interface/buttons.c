...
LOG_MODULE_REGISTER(MODULE, CONFIG_MESH_SENSOR_BUTTONS_LOG_LEVEL);

#define SCAN_INTERVAL CONFIG_MESH_SENSOR_BUTTONS_SCAN_INTERVAL
...
static int get_buttons(uint32_t *mask)
{
	for (size_t i = 0; i < ARRAY_SIZE(button_pins); i++) {
		...
		if (IS_ENABLED(CONFIG_MESH_SENSOR_BUTTONS_POLARITY_INVERSED)) {
			val = !val;
		}
...
{
	gpio_flags_t flags = (IS_ENABLED(CONFIG_MESH_SENSOR_BUTTONS_POLARITY_INVERSED) ?
...
static int callback_ctrl(bool enable)
{
	...
	for (size_t i = 0; (i < ARRAY_SIZE(button_pins)) && !err; i++) {
		if (enable) {
			 ...
			 */
			gpio_flags_t flag_irq = (IS_ENABLED(CONFIG_MESH_SENSOR_BUTTONS_POLARITY_INVERSED) ?
						(GPIO_INT_LEVEL_LOW) : (GPIO_INT_LEVEL_HIGH));

			err = gpio_pin_interrupt_configure(gpio_devs[button_pins[i].port],
							   button_pins[i].pin,
							   flag_irq);
		} else {
			err = gpio_pin_interrupt_configure(gpio_devs[button_pins[i].port],
...
static void scan_fn(struct k_work *work)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(button_pins); i++) {
		...
		if ((is_pressed != was_pressed) &&
		    (evt_limit < CONFIG_MESH_SENSOR_BUTTONS_EVENT_LIMIT)) {

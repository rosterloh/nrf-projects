...
LOG_MODULE_REGISTER(MODULE, CONFIG_MESH_SENSOR_BATTERY_CHARGER_LOG_LEVEL);


#define CSO_PERIOD_HZ		CONFIG_MESH_SENSOR_BATTERY_CHARGER_CSO_FREQ
...
static void error_check_handler(struct k_work *work)
{
	...
	if (cnt <= CSO_CHANGE_MAX) {
		int val = gpio_pin_get_raw(gpio_dev,
					   CONFIG_MESH_SENSOR_BATTERY_CHARGER_CSO_PIN);
		if (val < 0) {
...
static int cso_pin_control(bool enable)
{
	...
	if (enable) {
		...
		if (IS_ENABLED(CONFIG_MESH_SENSOR_BATTERY_CHARGER_CSO_PULL_UP)) {
			flags |= GPIO_PULL_UP;
		} else if (IS_ENABLED(CONFIG_MESH_SENSOR_BATTERY_CHARGER_CSO_PULL_DOWN)) {
...
{
	int charging = (IS_ENABLED(CONFIG_MESH_SENSOR_BATTERY_CHARGER_ENABLE_INVERSED))
			? (!enable) : (enable);

	return gpio_pin_set_raw(gpio_dev,
				CONFIG_MESH_SENSOR_BATTERY_CHARGER_ENABLE_PIN,
				charging);
}
...
static int init_fn(void)
{
	...
	gpio_init_callback(&gpio_cb, cs_change,
			   BIT(CONFIG_MESH_SENSOR_BATTERY_CHARGER_CSO_PIN));

...
LOG_MODULE_REGISTER(MODULE, CONFIG_MESH_SENSOR_POWER_MANAGER_LOG_LEVEL);


#define POWER_DOWN_ERROR_TIMEOUT K_SECONDS(CONFIG_MESH_SENSOR_POWER_MANAGER_ERROR_TIMEOUT)
#define POWER_DOWN_TIMEOUT_MS	 K_SECONDS(CONFIG_MESH_SENSOR_POWER_MANAGER_TIMEOUT)
...
static bool event_handler(const struct event_header *eh)
{
	...
	if (is_power_down_event(eh)) {
		...
		if (!usb_connected && (power_state == POWER_STATE_SUSPENDING)) {
			...
			if (IS_ENABLED(CONFIG_MESH_SENSOR_POWER_MANAGER_STAY_ON) ||
			    (connection_count > 0)) {
	...
	if (is_ble_peer_event(eh)) {
		...
		if (!usb_connected && (connection_count == 0) &&
		    (power_state == POWER_STATE_SUSPENDED) &&
		    !IS_ENABLED(CONFIG_MESH_SENSOR_POWER_MANAGER_STAY_ON)) {
	...
	if (IS_ENABLED(CONFIG_MESH_SENSOR_USB_ENABLE) && is_usb_state_event(eh)) {
